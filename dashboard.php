...
if(isset($approve_order))
{
	$order_id = $_POST['order_id'];
	if(mysqli_query($con,"update tblorder set fldstatus='Delivered' where fld_order_id='$order_id'"))
	{
		$approval_msg = "Order #$order_id has been approved and marked as Delivered!";
	}
	else
	{
		$approval_error = "Failed to approve order #$order_id";
	}
}
...
{
	$order_id = $_POST['order_id'];
	if(mysqli_query($con,"update tblorder set fldstatus='cancelled' where fld_order_id='$order_id'"))
	{
		$approval_msg = "Order #$order_id has been rejected and marked as cancelled!";
	}
	else
	{
		$approval_error = "Failed to reject order #$order_id";
	}
}
...
<div class="container">
	   ...
	   <ul class="nav nav-tabs nabbar_inverse" id="myTab" style="background:#ED2553;border-radius:10px 10px 10px 10px;" role="tablist">
		  ...
		  <li class="nav-item">
              <a class="nav-link" style="color:#BDDEFD;" id="orderstatus-tab" data-toggle="tab" href="#orderstatus" role="tab" aria-controls="orderstatus" aria-selected="false">
                  Order status
                  <?php
                  ...
                  if($pending_result['count'] > 0) {
                      echo "<span class='badge badge-danger' style='background-color:red;'>".$pending_result['count']."</span>";
                  }
                  ?>
              </a>
          </li>
	...
	?>			 
		...
		?>		   
			 ...
			 <div class="tab-pane fade" id="orderstatus" role="tabpanel" aria-labelledby="orderstatus-tab">
			   <?php if(isset($approval_msg)) { echo "<div class='alert alert-success'>$approval_msg</div>"; } ?>
			   <?php if(isset($approval_error)) { echo "<div class='alert alert-danger'>$approval_error</div>"; } ?>
               <table class="table">
			   <th>Order Id</th>
			   <th>Food Id</th>
			   ...
			   <td><?php echo $r_ff['foodname']; ?></td>
			   <td>Rs.<?php echo $rrr['fld_payment']; ?></td>
			   <td>
			   <?php
			   if($stat=="In Process")
			   {
			   ?>
			   <span style="color:orange;"><i class="fas fa-clock"></i>&nbsp;<?php echo $rrr['fldstatus']; ?> (Pending Approval)</span>
			   <?php
			   }
			   elseif($stat=="cancelled" || $stat=="Out Of Stock")
			   {
			   ?>
			   <span style="color:red;"><i class="fas fa-times-circle"></i>&nbsp;<?php echo $rrr['fldstatus']; ?></span>
			   <?php
			   }
			   else
			   {
			   ?>
			   <span style="color:green;"><i class="fas fa-check-circle"></i>&nbsp;<?php echo $rrr['fldstatus']; ?></span>
			   <?php
			   }
			   ?>
			   </td>
			   ...
			   <form method="post" style="display:inline;">
				   <input type="hidden" name="order_id" value="<?php echo $rrr['fld_order_id']; ?>">
				   <button type="submit" name="approve_order" class="btn btn-success btn-sm">Approve</button>
				   <button type="submit" name="reject_order" class="btn btn-danger btn-sm">Reject</button>
			   </form>
			   <?php
			   }
			   else
