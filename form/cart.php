...
<div class="middle" style="  padding:60px; border:1px solid #ED2553;  width:100%;">
	   ...
	   <ul class="nav nav-tabs nabbar_inverse" id="myTab" style="background:#ED2553;border-radius:10px 10px 10px 10px;" role="tablist">
		  ...
		  <li class="nav-item">
              <a class="nav-link" style="color:#BDDEFD;" id="orders-tab" data-toggle="tab" href="#orders" role="tab" aria-controls="orders" aria-selected="false">
                  Orders
                  <?php
                  $pending_orders = mysqli_query($con,"SELECT COUNT(*) as count FROM tblorder WHERE fld_email_id='$cust_id' AND fldstatus='In Process'");
                  $pending_result = mysqli_fetch_array($pending_orders);
                  if($pending_result['count'] > 0) {
                      echo "<span class='badge badge-warning' style='background-color:orange;'>".$pending_result['count']."</span>";
                  }
                  ?>
              </a>
          </li>
...
<!--tab 1 ends-->	   
            ...
            <div class="tab-pane fade" id="orders" role="tabpanel" aria-labelledby="orders-tab">
				...
				<th>Action</th>
				    <tbody>
					<?php
					$quer_res=mysqli_query($con,"select * from tblorder where fld_email_id='$cust_id' ORDER BY fld_order_id DESC");
					...
					?>
					   ...
					   {
					   ?>
					   <span style="color:orange;"><i class="fas fa-clock"></i>&nbsp;<?php echo $stat; ?> (Pending Approval)</span>
					   <?php
					   }
					   elseif($stat=="cancelled")
					   {
					   ?>
					   <span style="color:red;"><i class="fas fa-times-circle"></i>&nbsp;<?php echo $stat; ?></span>
					   <?php
					   }
					   elseif($stat=="Out Of Stock")
					   {
					   ?>
					   <span style="color:red;"><i class="fas fa-exclamation-triangle"></i>&nbsp;<?php echo $stat; ?></span>
					   <?php
					   }
					   else
					   {
					   ?>
					   <span style="color:green;"><i class="fas fa-check-circle"></i>&nbsp;<?php echo $stat; ?></span>
					   <?php
					   }
					   ?>
					   </td>
					   ...
					   {
					   ?>
					   <a href="#" onclick="del(<?php echo $roww['fld_order_id'];?>);"><button type="button" class="btn btn-danger btn-sm">Cancel Order</button></a>
					   <?php
					   }
					   else
					   {
